...
describe("published GenLayer dependencies (hackathon-required releases)", () => {
  it.each([
    ...
    const entry = lock.packages[`frontend/node_modules/${name}`] ?? lock.packages[`node_modules/${name}`];
  ...
  it("shares the SDK release between deployment and the frontend", () => {
    expect(root.devDependencies["genlayer-js"]).toBe(frontend.dependencies["genlayer-js"]);
    expect(lock.packages[""].devDependencies["genlayer-js"]).toBe(root.devDependencies["genlayer-js"]);
  });
});
